import { StockData, Market, StockPrefix, RawStockResponse, AStockData, HKStockData, USStockData } from './types.js';

const BASE_URL = 'https://qt.gtimg.cn';

// 根据市场获取前缀
function getPrefix(market: Market): StockPrefix {
  switch (market) {
    case 'sh': return 'sh';
    case 'sz': return 'sz';
    case 'hk': return 'r_hk';
...
// 解析原始响应
function parseRawResponse(text: string): RawStockResponse | null {
...
// 解析股票数据
function parseStockData(raw: RawStockResponse, market: Market): StockData {
  // 不过滤空字段，保留原始索引
...
// 解析A股数据
function parseAStock(fields: string[], market: 'sh' | 'sz'): AStockData {
  const getField = (idx: number): string => fields[idx] || '';
...
// 解析港股数据
function parseHKStock(fields: string[]): HKStockData {
  const getField = (idx: number): string => fields[idx] || '';
  const parseNum = (idx: number, defaultVal: number = 0): number => {
    const val = getField(idx);
    return val ? parseFloat(val) || defaultVal : defaultVal;
...
// 解析美股数据
function parseUSStock(fields: string[]): USStockData {
  const getField = (idx: number): string => fields[idx] || '';
  const parseNum = (idx: number, defaultVal: number = 0): number => {
    const val = getField(idx);
    return val ? parseFloat(val) || defaultVal : defaultVal;
